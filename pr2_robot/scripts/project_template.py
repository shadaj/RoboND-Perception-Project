...
def pr2_mover(object_list):
    ...
    for topick in object_list_param:
        found = False
        for found in object_list:
            if found.label == topick['name']:
                print("found " + found.label)
                points_arr = ros_to_pcl(found.cloud).to_array()
                centroid = np.mean(points_arr, axis=0)[:3]
                found = True
                break

        if not found:
            print(topick['name'] + " not found!")
            break
        ...
        try:
            ...
            object_name = String()
            object_name.data = topick['name']

            which_arm = String()
            if (topick['group'] == 'green'):
            ...
            place_pose = Pose()
            for boxOption in dropbox:
                if (boxOption['name'] == which_arm.data):
                    print('found arm: ' + which_arm.data)
                    box = boxOption
                    break
            ...
            yaml_arr.append(make_yaml_dict(test_scene_num, which_arm, object_name, pick_pose, place_pose))
        except rospy.ServiceException, e:
            print "Service call failed: %s"%e

    send_to_yaml('output_{}.yaml'.format(test_scene_num.data), yaml_arr)
